...
def orders_page(request):
    yolda = Cart.objects.filter(status=2)
    qaytarilgan = Cart.objects.filter(status=3)
...
def confirm_order(request, cart_id):
    if request.method == 'POST':
        ...
        cart.status = 4  
        cart.save()
        return HttpResponse("Buyurtma qabul qilindi")
    else:
...
def return_order(request, cart_id):
    if request.method == 'POST':
        ...
        cart.status = 3  
        cart.save()
        return HttpResponse("Buyurtma qaytarildi")
    else:
        return HttpResponse("Invalid Request")